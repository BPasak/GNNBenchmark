...
class GraphRes(BaseModel):
    ...
    def __init__(self, dataset, input_shape: torch.Tensor, num_outputs: int, pooling_size=(16, 12),
                 bias: bool = False, root_weight: bool = False, act: str = 'relu', grid_div: int = 8,
                 conv_type: str = 'fuse', distill: bool = False, character: str = None, drop: float = 0.0):
        ...
        if not self.distill:  # normal model
            if self.conv_type == 'fuse':
                ...
                self.conv1 = SplineConv(n[0], n[1], dim=dim, kernel_size=kernel_size, bias=bias,
                                        root_weight=root_weight)
                self.norm1 = BatchNorm(in_channels=n[1])
                self.conv2 = SplineConv(n[1], n[2], dim=dim, kernel_size=kernel_size, bias=bias,
                                        root_weight=root_weight)
                self.norm2 = BatchNorm(in_channels=n[2])

                self.conv3 = SplineConv(n[2], n[3], dim=dim, kernel_size=kernel_size, bias=bias,
                                        root_weight=root_weight)
                self.norm3 = BatchNorm(in_channels=n[3])
                self.conv4 = SplineConv(n[3], n[4], dim=dim, kernel_size=kernel_size, bias=bias,
                                        root_weight=root_weight)
                self.norm4 = BatchNorm(in_channels=n[4])

                self.conv5 = SplineConv(n[4], n[5], dim=dim, kernel_size=kernel_size, bias=bias,
                                        root_weight=root_weight)
                self.norm5 = BatchNorm(in_channels=n[5])
                # self.pool5 = MaxPooling(self.pooling_size, transform=Cartesian(norm=True, cat=False))
                self.pool5 = MaxPooling(self.pooling_size, img_shape=self.input_shape[:2],
                                        transform=Cartesian(norm=True, cat=False))

                self.conv6 = SplineConv(n[5], n[6], dim=dim, kernel_size=kernel_size, bias=bias,
                                        root_weight=root_weight)
                self.norm6 = BatchNorm(in_channels=n[6])
                self.conv7 = SplineConv(n[6], n[7], dim=dim, kernel_size=kernel_size, bias=bias,
                                        root_weight=root_weight)
            ...
            if self.character == 'teacher':
                ...
                if self.conv_type == 'spline':
                    ...
                    self.conv1 = PointNetConv(local_nn=Linear(1 + 3, 16, bias=False), global_nn=None,
                                              add_self_loops=False)
                    self.conv2 = PointNetConv(local_nn=Linear(n[4] + 3, n[5], bias=False), global_nn=None,
                                              add_self_loops=False)
                    self.conv3 = PointNetConv(local_nn=Linear(n[5] + 3, n[6], bias=False), global_nn=None,
                                              add_self_loops=False)
                    self.conv4 = PointNetConv(local_nn=Linear(n[6] + 3, n[7], bias=False), global_nn=None,
                                              add_self_loops=False)
    ...
    def quant(self, *, conv_f_dtype='uint8', conv_w_dtype='int8', fc_in_dtype='uint8', fc_w_dtype='int8',
              fc_out_dtype='int8'):
        for module in self.children():
            if isinstance(module, MyConvBNReLU):
    ...
    def forward(self, data: PyGBatch, **kwargs) -> torch.Tensor:
        ...
        if not self.distill:
            if self.conv_type == 'fuse':
                ...
                data = self.pool5(data.x, pos=data.pos, batch=data.batch, edge_index=data.edge_index,
