...
from Models.CleanEvGNN.asyncronous.base.base import make_asynchronous, add_async_graph
from Models.CleanEvGNN.asyncronous.base.utils import compute_edges, graph_changed_nodes, graph_new_nodes, cdist
from Models.CleanEvGNN.asyncronous.flops import compute_flops_conv

from Models.CleanEvGNN.my_conv import MyConv
from Models.CleanEvGNN.my_fuse import MyConvBNReLU
...
def __graph_initialization(module, x: torch.Tensor, edge_index: Adj = None, edge_attr=None, **kwargs):
    ...
    if module.conv_type == 'fuse':
        # Handle empty edge case - if no edges, just return zeros
    ...
    if module.asy_flops_log is not None:
        # Use edge_new instead of edge_index, and check if it's not empty
        if module.edge_new.numel() > 0:
            flops = __compute_flops(module, idx_new=module.edge_new.unique().long(), idx_diff=[], edges=module.edge_new)
            module.asy_flops_log.append(flops)
        else:
            # No edges, minimal flops (just for the node itself)
            module.asy_flops_log.append(0)
...
def __graph_processing(module, x: torch.Tensor, edge_index = None, edge_attr: torch.Tensor = None, **kwargs):
    ...
    if module.asy_flops_log is not None:
        if module.edge_new.numel() > 0:
            flops = __compute_flops(module, idx_new=[module.idx_new], idx_diff=[], edges=module.edge_new)
            module.asy_flops_log.append(flops)
        else:
            # No edges, minimal flops
            module.asy_flops_log.append(0)
