...
def normalize_time(t: torch.Tensor, eps: float = 1e-9) -> torch.Tensor:
    """Match AEGNN: per-sample min-max normalization on t only."""
    tmin = torch.min(t)
    tmax = torch.max(t)
...
class NCarsEventsGraphDataset(InMemoryDataset):
    ...
    def __init__(
        self,
        split_dir: str,
        k_ignored: int = 16,                     # not used (kept for compatibility)
        max_events: Optional[int] = None,        # optional hard cap before subsampling
        sensor_wh: Optional[Tuple[int,int]] = None,  # unused here (AEGNN doesn’t scale x,y)
        precompute_gcn_norm: bool = False,       # unused for AEGNN; we keep interface
        cache: bool = False,
        # AEGNN hyperparams:
        r: float = 3.0,
        d_max: int = 32,
    ...
    def get(self, idx: int) -> Data:
        ...
        #    NOTE: This uses *pixel* x,y directly like AEGNN; no xy normalization.
