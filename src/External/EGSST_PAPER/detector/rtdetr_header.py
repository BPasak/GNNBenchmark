...
from .rtdetr_head.rtdetr_converter import (
    convert_yolo_batch_to_targets_format,
    move_to_device,
    to_feats_list,
    convert_yolo_batch_to_coco_format,
)
...
class RTDETRHead(nn.Module):
    ...
    def enc_dec_loss(self, feats_list, target_format_lbl=None):
        # feats_list: list of feature maps from backbone
        encoder_output = self.encoder(feats_list)
    ...
    def coco_eval(self, tgt_tensor, post_results):
        assert self.training is False, "coco_eval should be used in eval mode"
        assert self.dataset_name in {"gen1", "gen4"}

        classes = ("car", "pedestrian") if self.dataset_name == "gen1" else (
            "pedestrian",
            "two-wheeler",
            "car",
        )
        ...
        coco_targets = convert_yolo_batch_to_coco_format(
            tgt_tensor,
        ...
        torch.save(
            coco_targets,
            "/home/catlab/py_code/gt4dvs/save_dir/processing/coco_targets.pt",
        )
        ...
        torch.save(
            res, "/home/catlab/py_code/gt4dvs/save_dir/processing/res.pt"
        )
        ...
        torch.save(
            coco_evaluator.coco_eval,
            "/home/catlab/py_code/gt4dvs/save_dir/processing/coco_evaluator_coco_eval.pt",
        )
        ...
        out_dict = {k: 0.0000 for k in out_keys}
        ...
        if hasattr(coco_evaluator, "coco_eval") and "bbox" in coco_evaluator.coco_eval:
            for idx, key in enumerate(out_keys):
                formatted_value = format(
                    coco_evaluator.coco_eval["bbox"].stats[idx], ".4f"
                )
    ...
    def forward(self, feats_dict, targets=None):
        # -------------------------------
        # HANDLE INPUT IMAGE SIZE
        # -------------------------------
        if self.dataset_name == "ncaltech":
            # New NCaltech API: returns DatasetInformation object
            dataset_info = NCaltech.get_info()
            scale_img_width, img_height = dataset_info.image_size
        else:
        ...
        # -------------------------------
        decoder_outputs, sum_loss, loss_dict = self.enc_dec_loss(
            feats_list, target_format_lbl
        )

        # TRAIN MODE → return losses
        if self.training:
            return sum_loss, loss_dict

        # EVAL MODE → return predictions / coco eval
        if self.use_coco_eval and not self.training:
            orig_target_sizes = torch.stack(
                [t["orig_size"] for t in target_format_lbl],
                dim=0,
            )
            torch.save(
                orig_target_sizes,
                "/home/catlab/py_code/gt4dvs/save_dir/processing/orig_target_sizes.pt",
            )
            post_results = self.postprocess(decoder_outputs, orig_target_sizes)
